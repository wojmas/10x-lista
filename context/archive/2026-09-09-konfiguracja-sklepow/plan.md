...
- **Weryfikacja ręczna wykryła błąd, którego testy nie łapały: `map(intval(...))` w `ShopController::store()`.** `Collection::map` woła wywołanie zwrotne z `(wartość, klucz)`, a `intval()` czyta drugi argument jako podstawę systemu liczbowego — identyfikator `"3"` pod indeksem 2 stawał się `0` i przypisanie kategorii wywracało się na kluczu obcym (HTTP 500 przy zaznaczeniu trzech kategorii). `AddShopTest` tego nie widział, bo podawał identyfikatory jako liczby całkowite, a wtedy `intval()` ignoruje podstawę; formularz HTML wysyła ciągi znaków. Naprawione jawną konwersją, a test przerobiony tak, żeby wysyłał ciągi znaków i trzy kategorie — bez poprawki kontrolera nowy test zgłasza `insert into "category_shop" ... values (0, 1)`.
- **Lokalny build Vite był nieaktualny (z 31 sierpnia) i nie zawierał `flex-wrap` ani `break-words`.** Przy sklepie z dwunastoma kategoriami plakietki nie zawijały się, tylko ścinały poza kartę. To nie jest usterka w kodzie zmiany — `public/build` jest w `.gitignore` i powstaje przy wdrożeniu — ale bez `npm run build` kryterium 1.9 wypada negatywnie lokalnie. Po przebudowie zawijanie działa, brak poziomego przewijania przy 375 px.
