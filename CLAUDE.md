...
- Use Docker for all development: `docker-compose up -d` (services: app, nginx, db)
- App available at http://localhost:8080 (nginx → PHP-FPM)
- Database: PostgreSQL 16 on port 5432 (user: `laravel`, password: `secret`, db: `lista_zakupow`)
- `.env` must set `DB_CONNECTION=pgsql` and `DB_HOST=db` when running in Docker (`.env.example` defaults to SQLite)

## Commands

- `composer setup` — first-time setup (installs deps, generates .env, key, runs migrations, builds frontend)
- `composer dev` — starts 4 concurrent processes: artisan serve, queue:listen, pail (logs), npm dev
...
- **Backend:** PHP 8.3+ / Laravel 13, Blade templates (server-rendered, no SPA)
- **Frontend:** Tailwind CSS 4 + Vite 8, no JS framework
- **Database:** PostgreSQL 16 (Docker), in-memory SQLite for tests
- **Auth:** Laravel built-in (admin creates users, no public registration)

## Conventions

- UI language: Polish (translations needed in `resources/lang/pl/`)
- Code style: Laravel defaults + Pint, 4-space indentation (PHP, JS), 2-space (YAML)
- Tests: Feature tests in `tests/Feature/`, Unit in `tests/Unit/`, use `RefreshDatabase` trait

## Key References

- @context/foundation/prd.md — product requirements (Polish, greenfield)
- @context/foundation/tech-stack.md — stack decisions from tech-stack selector
