{{--
    Formularz sklepu — dzielony przez dodawanie i edycję.

    Różnią się czterema rzeczami: adresem, metodą HTTP, napisem na przycisku i
    tym, co jest zaznaczone na starcie. Reszta (układ, podwójny odczyt błędów
    kategorii, pole nowej kategorii) musi być identyczna, bo to ten sam formularz
    — kopia oznaczałaby dwa miejsca do poprawienia przy każdej zmianie.

    @param string      $action      URL docelowy formularza
    @param string|null $method      'PUT' przy edycji, null przy dodawaniu
    @param string      $submitLabel Napis na przycisku zapisu
    @param \App\Models\Shop|null $shop Edytowany sklep; null przy dodawaniu
    @param \Illuminate\Support\Collection $categories Wszystkie kategorie
--}}
@php
    $method ??= null;
    $shop ??= null;
    // old() wygrywa po błędzie walidacji; przy pierwszym wejściu w edycję
    // decyduje bieżący stan sklepu, a przy dodawaniu — pusty zestaw.
    $checkedCategoryIds = old('category_ids', $shop?->categories->pluck('id')->all() ?? []);
@endphp
...
<form method="POST" action="{{ $action }}" class="p-6">
    ...
    <div class="mt-6">
        ...
        <div class="mt-2 space-y-1">
            @foreach ($categories as $category)
                <label for="category_{{ $category->id }}" class="flex items-center gap-3 py-2 cursor-pointer">
                    <input id="category_{{ $category->id }}" type="checkbox" name="category_ids[]"
                        value="{{ $category->id }}"
                        @checked(in_array($category->id, $checkedCategoryIds))
    ...
    <div class="mt-4">
        ...
        <p class="mt-1 text-sm text-gray-500">
            @if ($shop)
                Pole opcjonalne. Sklep bez żadnej kategorii zostanie na liście, ale nie trafi do rekomendacji.
            @else
